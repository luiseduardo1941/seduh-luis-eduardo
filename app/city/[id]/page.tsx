...
export default function CityPage() {
  ...
  const allHours = data.forecast.forecastday.flatMap((fd) => fd.hour);

  function findHourFor(refUtcIso: string) {
    const ref = DateTime.fromISO(refUtcIso);
    return allHours.find((h: any) => {
  ...
  function getWeatherIcon(text: string, size = 42) {
    ...
    if (t.includes("storm") || t.includes("thunder")) return <CloudLightning size={size} />;
  ...
  if (condition.includes("cloud")) { bgClass = "bg-[#C4C4C4]"; textClass = "text-black"; }
  else if (condition.includes("snow")) { bgClass = "bg-[#EAF6FF]"; textClass = "text-black"; }
  else if (condition.includes("rain")) { bgClass = "bg-[#506680]"; textClass = "text-white"; }
  else if (condition.includes("storm") || condition.includes("thunder")) { bgClass = "bg-[#2A3452]"; textClass = "text-white"; }
  else if (condition.includes("night") || condition.includes("moon")) { bgClass = "bg-[#0B1A30]"; textClass = "text-white"; }

  const finalTextColor = textClass === "text-white" ? "#FFFFFF" : "#000000";
    ...
    >
      <div className="w-full max-w-4xl text-center">
        ...
        <section className="flex justify-center gap-32 mb-20 flex-wrap">
          {tiles.map((t) => (
            <div key={t.label} className="flex flex-col items-center min-w-[120px]">
              <span className="text-sm opacity-80 mb-3">{t.label}</span>
              {t.hour ? (
                <>
                  <div className="mb-3">{getWeatherIcon(t.hour.condition.text, 64)}</div>
                  <span className="text-base font-medium">{Math.round(t.hour.temp_c)}°C</span>
                </>
              ) : (
